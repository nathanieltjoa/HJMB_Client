...
const getDLaporanRetur = gql`
  query getDLaporanRetur(
    $id: String
  ){
    getDLaporanRetur(
      id: $id
...
export default function DetailRetur(props) {
    let history = useHistory();
    const location = useLocation();
    const [dataLaporan, setDataLaporan] = useState([]);
    const [newUri, setNewUri] = useState("");
    const { loading, data, refetch} = useQuery(getDLaporanRetur,{
        variables: {
            id: dataLaporan?.id
        }
    });
    ...
    }else if(data.getDLaporanRetur === null){
                ...
                <Col className="col-md-12">
                    ...
                    <div className='tableContainer'>
                        <table size='string' className="table" aria-label="simple table">
                            ...
                            <tbody>
                                {
                                    data.getDLaporanRetur.map((laporan,index) =>(
                                        <tr key={index} >
    ...
    return (
        <Container className="containerKu">
            <Row>
                <Col>
                    <BiIcons.BiArrowBack size="50" onClick={() => history.push({pathname: '/laporan/pipa retur'})} className="iconBack"/>
                </Col>
            </Row>
            <Row className="bg-white justify-content-center">
                <Col><h1 className="text-center">Master Laporan Pipa Retur</h1></Col>
            </Row>
                    ...
                    <Card>
                        ...
                        <Card.Body>
                            <Card.Text className="parent">
                                <p className="childLeft">Nomor Nota</p>
