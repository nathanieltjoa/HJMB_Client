...
const getLaporanRetur = gql`
query getLaporanRetur(
  $page: Int
  $limit: Int 
  $orderBy: String 
  $bulan: MyDate
    ...
    count rows{
      id karyawan{nama} idNota foto keterangan createdAt
    }
  }
}
`;

export default function Retur(props) {
    ...
    const goToDetail = (laporan) => {
        console.log("asd");
        history.push({
            pathname: '/laporan/detail pipa retur',
    ...
    }else if(dataLaporan.getLaporanRetur.count){
      var jml = Math.ceil(dataLaporan.getLaporanRetur.count / limit);
      pageKu.push(
        <ReactPaginate
          key={1}
    ...
    }else if(dataLaporan.getLaporanRetur.rows.length === 0){
        ...
        dataKu.push(
            <div className='tableContainer'>
                <table size='string' className="table" aria-label="simple table">
                    ...
                    <tbody>
                        {
                            dataLaporan.getLaporanRetur.rows.map((laporan,index) =>(
                                <tr key={index} >
                                    <td data-label="Nomor Nota">{laporan.idNota}</td>
    ...
    return (
        <Container className="containerKu">
            <Row className="bg-white justify-content-center">
                <Col>
                    <h1 className="text-center">Master Laporan Pipa Retur</h1>
                </Col>
            </Row>
            <Row>
