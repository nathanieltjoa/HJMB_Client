...
export default function Register(props) {
    ...
    const [registerKaryawanku] = useMutation(registerKaryawan,{
        ...
        onCompleted(data){
            setErrors({});
            registerUserku({variables:{
                id: data.registerKaryawan.id,
                username: variables.username,
                idJabatan: variables.idJabatan
            }})
        }
    })

    const [registerUserku] = useMutation(registerUser,{
        update(_,res){
            console.log(res)
        },
        onError: (err) => {
        ...
        onCompleted(data){
            console.log(data);
            setSuccess({
                username: `Username: ${data.registerUser.username}`,
                password: `Password: ${data.registerUser.passwordRaw}`
            })
        }
      })
    const register = () =>{
